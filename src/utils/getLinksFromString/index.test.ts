...
describe('getLinksFromString function', () => {
  it('получение списка строк с несколькими ссылками', () => {
    const text1 = 'Новое мероприятие доступно по <a href="https://lzone.secret-agents.ru/events/45">ссылке</a> Также, более подробдую информацию можете найти на сайте <a href="https://lzone.secret-agents.ru/info">https://lzone.secret-agents.ru/info</a>';
    ...
    expect(getLinksFromString(text1)).toEqual([
      ...
      { text: ' Также, более подробдую информацию можете найти на сайте ', url: null },
      { text: 'https://lzone.secret-agents.ru/info', url: 'https://lzone.secret-agents.ru/info' },
    ]);
  });
  ...
  it('получение списка строк с несколькими ссылками при передачи многострочного текста', () => {
    ...
    expect(getLinksFromString(text)).toEqual([
      ...
      {
        text: 'ссылке', url: 'https://lzone.secret-agents.ru/special_offers/185'
      },
    ])
  });
